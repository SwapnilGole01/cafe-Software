...
export default function AdminLogin({ onLoginSuccess }: AdminLoginProps) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      onLoginSuccess({
        id: 999999,
        email: result.user.email || "google-user@cafe.com",
        role: "owner",
      });
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 font-sans">
      ...
      >
        <div className="bg-slate-900 p-8 text-center text-white relative">
          ...
          <h1 className="text-2xl font-semibold tracking-tight">DEV STUDIO</h1>
        ...
        <div className="p-8">
          ...
          <div className="mt-8 border-t border-slate-100 pt-4 text-center">
            <p className="text-[10px] text-slate-400 leading-normal">
              Default owner credentials:<br />
              <span className="font-mono bg-slate-50 px-1 py-0.5 rounded text-slate-600 font-bold">admin@cafe.com</span> /{" "}
              <span className="font-mono bg-slate-50 px-1 py-0.5 rounded text-slate-600 font-bold">adminpassword</span>
            </p>
          </div>
        </div>
      </motion.div>
